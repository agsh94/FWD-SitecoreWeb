...
{
    public class ArticleCardResolverTest
    {
        [Theory]
        [AutoDbData]
        public void ArticleCardTest(Db db, DbItem item)
        {
            ...
            using (new Sitecore.Sites.SiteContextSwitcher(fakeSite))
            {
                ...
                using (RenderingContext.EnterContext(rendering, datasourceItem))
                {
                    ...
                    {
                        ArticleCardContentResolver articleCardContentResolver = new ArticleCardContentResolver(renderingContentsResolver.Object);
                        articleCardContentResolver.IncludeServerUrlInMediaUrls = true;
                        JObject data = (JObject)articleCardContentResolver.ResolveContents(rendering, renderingconfiguration);
                        Assert.NotNull(data["link"]);
        ...
        {
            var name = item.Name;
            var path = Directory.GetCurrentDirectory() + "\\Data\\ArticleCardResolver.json";
