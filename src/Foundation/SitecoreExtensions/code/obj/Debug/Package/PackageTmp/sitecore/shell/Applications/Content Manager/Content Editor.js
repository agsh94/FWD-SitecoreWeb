...
scContentEditor.prototype.toggleSection = function (id, sectionName) {
	...
	if(sectionName.includes("custom")){
		var parent = el.parentElement;
		var sections = parent.children;
		var currentid = el.id;
		
		for(var count=0;count<sections.length;count++){
			var elem = sections[count];
			if(sections[count].tagName == "TABLE"&& el.nextSibling!=sections[count]){
				sections[count].style.display = "none";
			}else if(sections[count].tagName == "DIV" && sections[count].id!=currentid){
				sections[count].className = "scEditorSectionCaptionCollapsed";
				var icon = jQuery(sections[count]).find("img");
				icon.attr("src",icon.attr("src").replace( "/accordion_up","/accordion_down"));
			}
		}
	}
