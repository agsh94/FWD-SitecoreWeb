(function(speak) {
  speak.component(["itemJS", "bclSession"], function (itemJS) {
    ...
    var subscribe = function(control) {
      control.on("change:SearchQuery change:PageSize change:PageIndex change:SelectedFacets change:RootItemId " +
          "change:SearchConfigItemId change:Sorting change:ShowHiddenItems", control.loadData, control);
    };
    ...
    var getOptions = function(control) {
	  ...
	  if($.ajaxSettings.url.includes("FormsBuilder/Pages/Forms"))
	  {
		  var cookieValue = "";
          ...
          cookieArray = cookie.split(";");
			for (var i = 0; i < cookieArray.length; i++) {
			  ...
			  if (cookieItem[0] === "FormRootId") {
                       cookieValue = cookieItem[1];
                    }
                }
		  options.root = cookieValue;
	  }
