...
{
    public class GroupLocationFacilitiesTests
    {
        [Fact]
        public void Item_Null_Condition_Test()
        {
            // arrange
            var groupLocationFacilities = new GroupLocationFacilities();

            // act
            var result = groupLocationFacilities.ComputeFieldValue(null);
        ...
        public void Item_Standard_Values_Condition_Test([Content] Item item)
        {
            ...
            // act
            var result = groupLocationFacilities.ComputeFieldValue(new Sitecore.ContentSearch.SitecoreIndexableItem(itemStdValues));
        ...
        public void Item_Base_Template_Condition_Test_False([Content] Item root, [Content] ItemTemplate template)
        {
            ...
            // act
            var result = groupLocationFacilities.ComputeFieldValue(new Sitecore.ContentSearch.SitecoreIndexableItem(anySitecoreItem));
        ...
        public void Item_Base_Template_Condition_Test_True(string baseTemplateId)
        {
            ...
            using (Db db = new Db
            {
                ...
                {
                   new DbField(SearchConstant.LocationFacilitiesGroupField) { Type = "Lookup" }
                },
            ...
            })
            {
                ...
                ReferenceField referenceField = item.Fields[new ID(SearchConstant.LocationFacilitiesGroupField)];

                item.Editing.BeginEdit();
                referenceField.Value = dummyTagItem.ID.ToString();
                item.Editing.EndEdit();

                var groupLocation = new GroupLocationFacilities();

                // act
                var result = groupLocation.ComputeFieldValue(new Sitecore.ContentSearch.SitecoreIndexableItem(item));
        ...
        public class ItemTemplate : DbTemplate
        {
            public ItemTemplate()
            {
                ...
                this.Add("groupLocationFacilities");
            }
        }
    }
}
