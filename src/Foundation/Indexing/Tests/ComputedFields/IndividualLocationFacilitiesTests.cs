...
{
    public class IndividualLocationFacilitiesTests
    {
        [Fact]
        public void Item_Null_Condition_Test()
        {
            // arrange
            var individualLocationFacilities = new IndividualLocationFacilities();

            // act
            var result = individualLocationFacilities.ComputeFieldValue(null);
        ...
        public void Item_Standard_Values_Condition_Test([Content] Item item)
        {
            ...
            // act
            var result = individualLocationFacilities.ComputeFieldValue(new Sitecore.ContentSearch.SitecoreIndexableItem(itemStdValues));
        ...
        public void Item_Base_Template_Condition_Test_False([Content] Item root, [Content] ItemTemplate template)
        {
            ...
            // act
            var result = individualLocationFacilities.ComputeFieldValue(new Sitecore.ContentSearch.SitecoreIndexableItem(anySitecoreItem));
        ...
        public void Item_Base_Template_Condition_Test_True(string baseTemplateId)
        {
            ...
            using (Db db = new Db
            {
                ...
                {
                   new DbField(SearchConstant.LocationFacilitiesIndividualField) { Type = "Lookup" }
                },
            ...
            })
            {
                ...
                ReferenceField referenceField = item.Fields[new ID(SearchConstant.LocationFacilitiesIndividualField)];

                item.Editing.BeginEdit();
                referenceField.Value = dummyTagItem.ID.ToString();
                item.Editing.EndEdit();

                var individualLocation = new IndividualLocationFacilities();

                // act
                var result = individualLocation.ComputeFieldValue(new Sitecore.ContentSearch.SitecoreIndexableItem(item));
        ...
        public class ItemTemplate : DbTemplate
        {
            public ItemTemplate()
            {
                ...
                this.Add("individualLocationFacilities");
            }
        }
    }
}
