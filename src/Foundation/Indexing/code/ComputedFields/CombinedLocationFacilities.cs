...
namespace FWD.Foundation.Indexing.ComputedFields
{
    ...
    public class CombinedLocationFacilities : AbstractComputedIndexField
    {
        public override object ComputeFieldValue(IIndexable indexable)
        {
            try
            {
                ...
                if (item.IsDerived(new ID(SearchConstant.HospitalTemplateID)) || item.IsDerived(new ID(SearchConstant.BranchDetailsTemplateID)))
                {
                    ...
                    return ComputedFieldHelper.MergeList(facilityIndividual, facilityGroup);
                }
                return new List<string>();
            }
            catch (Exception ex)
            {
                Logger.Log.Error("Error while generating CombinedLocationFacilities computed field " + ex);
                return new List<string>();
            }
        }
    }
